...
export const useQuotesStore = defineStore('quotes', () => {
  ...
  const filtered = computed(() => {
    return allQuotes.value.filter(q => {
      ...
      const colorMatch = selectedColor.value === '' || q.color === parseInt(selectedColor.value)
      const sq = searchQuery.value.toLowerCase()
      const textMatch = !sq || q.text.toLowerCase().includes(sq) || q.book.toLowerCase().includes(sq)
  ...
  const books = computed(() => {
    ...
    allQuotes.value.forEach(q => { if (!map[q.book]) map[q.book] = q.volumeId })
    return Object.entries(map).sort(([a], [b]) => a.localeCompare(b)).map(([title, volumeId]) => ({ title, volumeId }))
  })
  ...
  function coverUrl(volumeId: string): string | null {
    ...
    const path = volumeId.replace(/^file:\/\/\//, '/').replace(/^file:\/\//, '')
  ...
  function getLearnedSubnets(): string[] {
    try { return JSON.parse(localStorage.getItem(SUBNETS_KEY) || '[]') } catch { return [] }
  }
  ...
  function ingest(data: any[], live: boolean) {
    ...
    try { coverCache.value = JSON.parse(localStorage.getItem(COVERS_KEY) || '{}') } catch {}
    try { doneState.value = JSON.parse(localStorage.getItem(DONE_KEY) || '{}') } catch {}

    if (live) fetchAndCacheCovers()
  }

  async function fetchAndCacheCovers() {
    const toFetch = books.value.filter(b => b.volumeId && !coverCache.value[b.volumeId])
  ...
  // ── Connection ─────────────────────────────────────────
  async function tryUrl(url: string): Promise<{ url: string; data: any } | null> {
    try {
  ...
  async function connect(rawIp: string) {
    ...
    try {
      ...
      const subnet = full.replace(IP_SUFFIX, '').replace(/:\d+$/, '').split('.').slice(0, 3).join('.')
      saveSubnet(subnet)
      ingest(data, true)
    } catch (e: any) {
  ...
  async function scan() {
    ...
    const candidates = subnets.map(s => `http://${s}${IP_SUFFIX}/annotations.json`)
    const results = await Promise.all(candidates.map(tryUrl))
    const match = results.find(r => r !== null)
    isLoading.value = false

    if (match) {
      const subnet = match.url.replace('http://', '').replace(`${IP_SUFFIX}/annotations.json`, '')
  ...
  async function updateColor(bookmarkId: string, color: number) {
    ...
    } catch (e) { console.error('updateColor failed', e) }
  }

  return {
    // State
    allQuotes, isLive, isConnected, isLoading, connError,
    selectedBook, selectedColor, searchQuery, coverCache, doneState,
    // Getters
    filtered, books, savedIp,
    // Actions
    connect, scan, tryReconnect, loadFromCache, reset, ingest,
    toggleDone, isDone, updateColor,
    getCachedCover, coverUrl, getFullIp,
    COLOR_MAP, COLOR_NAMES, IP_SUFFIX,
  }
})